-- Surface room lifecycle state (status / deactivated_at) in the snapshot and
-- keep the existing contract otherwise. Owners can still open a deactivated
-- room from the admin workspace; non-owner viewers go through
-- get_room_join_preview which now hides deactivated rooms.
