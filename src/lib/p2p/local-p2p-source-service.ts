...
import { LocalP2pError, type LocalP2pDescriptor } from "./local-p2p-contracts";
import { getBrowserLocalP2pRuntime, type LocalP2pRuntime } from "./local-p2p-runtime";
...
export function createLocalP2pSourceService(client: SupabaseClient<Database>, runtime: LocalP2pRuntime, mediaQueue: Pick<MediaQueueService, "addMedia"> = createMediaQueueService(client)): LocalP2pSourceService {
  ...
  async function resolveSource(roomId: string, mediaId: string): Promise<LocalP2pDescriptor> {
    ...
    const row = data?.[0];
    if (error || !row?.info_hash || !row.magnet_uri || !row.file_name || !row.file_size) throw new LocalP2pError("p2p_invalid_descriptor", "The authorized device-stream descriptor is unavailable.", { cause: error ?? undefined });
    return Object.freeze({ infoHash: row.info_hash, magnetUri: row.magnet_uri, fileName: row.file_name, fileSize: row.file_size, mimeType: null });
  }
  async function resumeDeviceStream(expected: LocalP2pDescriptor, file: File): Promise<LocalP2pDescriptor> {
    const descriptor = await runtime.seedLocalFile(file);
    if (
      descriptor.infoHash !== expected.infoHash ||
      descriptor.fileSize !== expected.fileSize
    ) {
      await runtime.leaveLocalStream(descriptor.infoHash).catch(() => undefined);
...
export function getBrowserLocalP2pSourceService(): LocalP2pSourceService {
  browserSourceService ??= createLocalP2pSourceService(createBrowserSupabaseClient(), getBrowserLocalP2pRuntime());
  return browserSourceService;
}
export function resetBrowserLocalP2pSourceService(): void { browserSourceService = null; }
